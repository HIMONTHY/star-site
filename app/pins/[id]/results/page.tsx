...
export default function ResultsPage() {
  ...
  return (
    <div className="relative min-h-screen">
      {/* Background Glow Blobs for the blur to catch */}
      <div className="fixed top-[-10%] left-[-10%] w-[40%] h-[40%] bg-blue-900/10 blur-[120px] rounded-full pointer-events-none" />
      <div className="fixed bottom-[-10%] right-[-10%] w-[40%] h-[40%] bg-green-900/10 blur-[120px] rounded-full pointer-events-none" />

      <div className="relative z-10 flex flex-col md:flex-row gap-8 max-w-[1200px] mx-auto py-12 px-6">
        
        {/* 1. SIDEBAR with BLUR */}
        <aside className="w-full md:w-60 flex flex-col gap-2">
          <div className="bg-[#111418]/70 backdrop-blur-xl border border-[#ffffff0a] rounded-xl p-4 shadow-2xl">
            <div className="flex items-center gap-3 p-3 bg-[#ffffff0d] text-white rounded-lg cursor-pointer border border-[#ffffff0a]">
        ...
        <main className="flex-1">
          <div className="mb-8">
            ...
            <h1 className="text-3xl font-bold text-white mb-1 tracking-tight">Scan results</h1>
              ...
              {activeTab === "accounts" && (
                <ResultCard title="Roblox Accounts">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-gray-800 rounded-lg flex-shrink-0 bg-gradient-to-br from-gray-700 to-black border border-[#ffffff1a]" />
                    <div>
                      <p className="text-[#2ea043] font-bold text-sm leading-tight">{results.robloxAccount?.username || "Unknown"}</p>
                      <p className="text-[10px] text-gray-500 break-all mt-1">{results.robloxAccount?.profile}</p>
                    </div>
                  </div>
                </ResultCard>
              )}
              ...
              {activeTab === "security" && (
                <>
                  <ResultCard title="Roblox Logs (Flags)">
                    <pre className="text-[12px] leading-relaxed text-gray-300 font-mono whitespace-pre-wrap">{results.robloxLogs || "No flags found."}</pre>
                  </ResultCard>
  ...
  return (
    <div 
      ...
      className={`flex items-center gap-3 p-3 rounded-lg cursor-pointer transition-all border ${
        isActive 
        ? "bg-[#2386361a] text-[#2ea043] border-[#23863633] backdrop-blur-md" 
        : "text-gray-400 border-transparent hover:text-white hover:bg-[#ffffff0a]"
      }`}
    >
...
function ResultCard({ title, children }: any) {
  return (
    <div className="bg-[#111418]/60 backdrop-blur-xl border border-[#ffffff0a] rounded-xl p-6 shadow-xl">
      <div className="flex items-center gap-3 mb-5">
        <div className="bg-[#2386361a] text-[#2ea043] p-2 rounded-lg border border-[#23863633]">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"></path></svg>
        </div>
        <h3 className="text-white font-semibold text-base">{title}</h3>
      </div>
      <div className="bg-[#00000040] border border-[#ffffff0a] rounded-lg p-4">
        {children}
      </div>
    </div>
  );
