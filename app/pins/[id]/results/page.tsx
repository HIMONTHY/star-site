            ...
            {activeTab === "accounts" && (
              <ResultCard title="Roblox Accounts">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-blue-400 font-bold text-sm leading-tight">{results.robloxAccount?.username || "N/A"}</p>
                    <p className="text-[10px] text-white/40 break-all mt-1">{results.robloxAccount?.profile}</p>
                  </div>
                </div>
              </ResultCard>
            )}
...
// ... (TabItem and ResultCard components remain exactly the same as you provided)
function TabItem({ label, isActive, onClick }: any) {
  return (
    <div onClick={onClick} className={`flex items-center gap-3 px-4 py-3 rounded-xl font-semibold cursor-pointer border transition ${isActive ? "bg-blue-500/15 border-blue-500/30 text-blue-200 shadow-[0_25px_80px_rgba(59,130,246,0.12)]" : "bg-transparent border-transparent text-white/60 hover:bg-white/5 hover:text-white"}`}>
      <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={isActive ? "text-blue-300" : "text-white/40"}><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"></path></svg>
