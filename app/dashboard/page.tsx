...
type PinRow = {
  ...
  createdAt: string;
  hasResults: boolean;
  results?: any; // Added for modal data
};
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    loadPins();
    const t = setInterval(loadPins, 4000); // Set to 4s as per UI text
        ...
        <div className="mx-auto max-w-7xl px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 text-sm">
            ...
            {loggedIn ? (
              ...
              <a href="/api/auth/login" className="ml-2 flex items-center gap-2 rounded-xl bg-indigo-500 px-4 py-2 font-semibold text-white shadow-[0_15px_60px_rgba(99,102,241,0.35)]">→ Discord login</a>
            )}
          </div>
        </div>
      </div>
              ...
              {pins.length === 0 ? (
                ...
                pins.map((p) => (
                  <div key={p.id} className="grid grid-cols-4 py-4 text-sm items-center hover:bg-white/5 transition rounded-xl">
                    ...
                    <div>
                      {p.hasResults ? <Badge tone="good">Finished</Badge> : <Badge tone="neutral">Pending</Badge>}
                    </div>
                    ...
                    <div className="flex justify-end">
                      ...
                      {p.hasResults ? (
                        ...
                        <span className="text-white/20 pr-6">—</span>
                      )}
                    </div>
                  </div>
                ))
      ...
      {activeResults && (
        <div className="results-overlay">
          <div className="results-modal">
            ...
            <pre className="bg-black/50 p-4 rounded-lg overflow-auto max-h-[50vh] text-xs font-mono mb-4 border border-white/5">
              {JSON.stringify(activeResults.results || { status: "Scan complete, no flags found." }, null, 2)}
            </pre>
            <div className="flex justify-end">
              <button onClick={closeResults} className="px-6 py-2 bg-blue-600 hover:bg-blue-500 rounded-lg font-semibold transition">Close</button>
            </div>
          </div>
        </div>
      )}
...
/* === REST OF YOUR COMPONENTS (Particles, Icons, Badge, etc) UNTOUCHED === */

function SidebarItem({ label, icon, active }: { label: string; icon?: React.ReactNode; active?: boolean }) {
  return (
    <div className={["mb-2 rounded-xl px-4 py-3 font-semibold cursor-pointer border transition flex items-center gap-3", active ? "bg-blue-500/15 border-blue-500/30 text-blue-200 shadow-[0_25px_80px_rgba(16,185,129,0.12)]" : "bg-transparent border-white/10 text-white/70 hover:bg-white/5 hover:text-white"].join(" ")}>
...
function StatPremium({ label, value }: { label: string; value: number }) {
  return (
    <div className="rounded-2xl border border-white/10 bg-[#0f141b]/75 backdrop-blur p-5 hover:-translate-y-0.5 transition shadow-[0_30px_120px_rgba(0,0,0,0.45)]">
      ...
      <div className="mt-3 h-1 rounded-full bg-white/10 overflow-hidden"><div className="h-full w-2/3 bg-blue-500/40" /></div>
    </div>
  );
}
