...
export default function DashboardPage() {
  ...
  async function loadPins() {
    try {
      // Adding a timestamp to the URL helps bypass browser cache
        ...
        <section>
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div>
              ...
              <p className="mt-1 text-white/60">Pins refresh automatically every minute.</p>
            </div>
          ...
          <div className="mt-8 rounded-2xl border border-white/10 bg-[#0f141b]/75 backdrop-blur p-6 shadow-[0_30px_120px_rgba(0,0,0,0.45)]">
            <div className="flex items-center justify-between mb-4">
              <div className="text-sm text-white/70">Pin Status</div>
              <button onClick={loadPins} className="rounded-lg bg-black/40 px-4 py-2 text-xs hover:bg-black/60 border border-white/10 transition">
                ↻ Force Refresh
              </button>
            </div>
...
/* Updated Badge for Green "Finished" state */
...
// ... SidebarItem, StatPremium, Particles, and Icons remain the same
